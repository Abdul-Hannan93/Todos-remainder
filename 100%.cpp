...
void iDraw()
{
	...
	if (settings_mode == 0 && mood == 0 && ringtone_mood == 0 && show_reminder == 0 && add_task == 0 && guideline == 0 && search == 0 && nick == 0)
	{
		...
		if (isCalled == 1)
		{
			if ((hours == hour) && (minutes == minute) && (days == day) && ((months + 1) == month) && ((years + 1900) == year))
			{
				...
				iText(320, 50, red, GLUT_BITMAP_HELVETICA_18);
	...
	if (settings_mode == 0 && mood == 0 && ringtone_mood == 0 && show_reminder == 0 && guideline == 1)
	{
		...
		iText(172, 260, "(Event Name)&(dd/mm/yr)&(hr:min)", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(182, 220, "i.e.First Ramadan&25/3/2023&5:30", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(175, 180, "Time must be in international format.", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(215, 140, "You can set multiple events.", GLUT_BITMAP_TIMES_ROMAN_24);
