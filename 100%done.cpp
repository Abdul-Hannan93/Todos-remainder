...
void CheckAlarm()
{
	...
	if ((hr2 == hours) && (min2 == minutes)&&(tasty==1))
	{
		music_on2 = 1;
		follow = 1;
		musicfunc3();
...
void iDraw()
{
	...
	if (settings_mode == 0 && mood == 0 && ringtone_mood == 0 && show_reminder == 0 && add_task == 0 && guideline == 0 && search == 0 && nick == 0 && dailyA == 0)
	{
		...
		if (isCalled == 1)
		{
			...
			if ((hr2 == hours) && (min2 == minutes) && (follow == 1)&&(tasty==1))
			{
	...
	if (settings_mode == 0 && mood == 0 && ringtone_mood == 0 && show_reminder == 0 && guideline == 1)
	{
		...
		iText(280, 360, "Input Format", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(172, 330, "(Event Name)&(dd/mm/yr)&(hr:min)", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(182, 300, "i.e.First Ramadan&25/3/2023&5:30", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(200, 270, "(Event Name)&(description)", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(182, 240, "i.e Ramadan & Start of holy month", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(175, 210, "Time must be in international format.", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(215, 180, "You can set multiple events.", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(200, 150, "Press UP button to turn off alarm", GLUT_BITMAP_TIMES_ROMAN_24);
